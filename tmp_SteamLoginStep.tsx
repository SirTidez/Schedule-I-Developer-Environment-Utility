...
const SteamLoginStep: React.FC<SteamLoginStepProps> = ({
  ...
  const [hasStored, setHasStored] = useState<{ exists: boolean; info?: { encryptedAt: string; lastUsed: string } } | null>(null);
  const [storedPassword, setStoredPassword] = useState('');
  const [useMasterForStorage, setUseMasterForStorage] = useState(false);
  const [masterPassword, setMasterPassword] = useState('');
  const [masterPasswordConfirm, setMasterPasswordConfirm] = useState('');

  // SteamCMD progress listener for Steam Guard messages
  useEffect(() => {
    ...
    return () => window.electronAPI.removeSteamCMDProgressListener();
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.electronAPI?.steamLogin?.hasCredentials?.();
        if (res) setHasStored(res);
      } catch {}
    })();
  }, []);
  ...
  const useStoredCredentials = async () => {
    if (!steamCMDPath) {
      setLoginStatus(prev => ({ ...prev, error: 'SteamCMD path not configured' }));
      return;
    }
    if (!storedPassword.trim()) {
      setLoginStatus(prev => ({ ...prev, error: 'Enter decryption password to use stored credentials' }));
      return;
    }
    setLoginStatus(prev => ({ ...prev, isLoggingIn: true, error: null, progress: 'Decrypting stored credentials...' }));
    try {
      const res = await window.electronAPI?.steamLogin?.getCredentials?.(storedPassword);
      if (!res || !res.success || !res.credentials) {
        setLoginStatus(prev => ({ ...prev, isLoggingIn: false, error: res?.error || 'Failed to load stored credentials' }));
        return;
      }
      ...
      await handleLogin();
      try { await window.electronAPI?.steamLogin?.updateLastUsed?.(storedPassword); } catch {}
    } catch (e) {
      setLoginStatus(prev => ({ ...prev, isLoggingIn: false, error: e instanceof Error ? e.message : String(e) }));
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (stayLoggedIn) {
          try {
            const payload: any = { username, password, stayLoggedIn: true };
            if (useMasterForStorage) {
              if (!masterPassword || masterPassword !== masterPasswordConfirm) {
                setLoginStatus(prev => ({ ...prev, error: 'Master password missing or does not match confirmation' }));
              } else {
                payload.mode = 'master-password';
                payload.masterPassword = masterPassword;
                await window.electronAPI?.steamLogin?.storeCredentials?.(payload);
              }
            } else {
              payload.mode = 'steam-password';
              await window.electronAPI?.steamLogin?.storeCredentials?.(payload);
            }
          } catch {}
        }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stored Credentials */}
      {hasStored?.exists && !loginStatus.isSuccess && !steamProcess?.isRunning && (
        <div className="bg-gray-800/50 border border-gray-600 rounded-lg p-4 mb-4">
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm text-gray-300">
              Stored credentials found {hasStored.info?.lastUsed ? `(last used: ${hasStored.info.lastUsed})` : ''}
            </div>
          </div>
          <div className="flex space-x-2 items-center">
            ...
            <button onClick={() => useStoredCredentials()} disabled={loginStatus.isLoggingIn} className="btn-secondary">
              Use Stored
            </button>
          </div>
        </div>
      ...
      {hasStored?.exists && !loginStatus.isSuccess && !steamProcess?.isRunning && (
        <div className="bg-gray-800/50 border border-gray-600 rounded-lg p-4 mb-4">
          ...
          <div className="flex space-x-2 items-center">
            <input
              type={showStoredPassword ? 'text' : 'password'}
              placeholder={hasStored.info?.mode === 'master-password' ? 'Enter master password' : 'Enter Steam password'}
              value={storedPassword}
              onChange={(e) => setStoredPassword(e.target.value)}
              className="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              disabled={loginStatus.isLoggingIn}
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 space-y-3">
              <div className="flex items-center space-x-3">
                <input
                  id="use-master"
                  type="checkbox"
                  checked={useMasterForStorage}
                  onChange={(e) => setUseMasterForStorage(e.target.checked)}
                  className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500"
                  disabled={loginStatus.isLoggingIn}
                />
                <label htmlFor="use-master" className="text-sm text-gray-300">
                  Use separate master password for storage
                </label>
              </div>
              {useMasterForStorage && (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="block text-sm mb-1">Confirm master password</label>
                    <input
                      type="password"
              ...
              <p className="text-xs text-gray-400">
                If unchecked, your Steam password acts as the decryption key and is never stored.
                If checked, your credentials are encrypted with your separate master password.
              </p>
            </div>
          )}
            ...
            <div>
              ...
              {hasStored?.exists && (
                <p className="text-xs text-yellow-200 mt-2">
                  Tip: If you saved credentials earlier, you can use the "Use Stored" section above to auto-fill
                  after entering your decryption password.
                </p>
              )}
            </div>
          </div>
